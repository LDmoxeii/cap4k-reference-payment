...
`CloseExpiredPayment` 的规范化 payload 绑定 `merchantId + paymentId`。`ExecuteSettlement` 的规范化 payload 绑定 `merchantId + settlementId + executionId + executionChannelId`：完全相同的 identity/key 重放返回原 operation 和 execution；同 key 改变任一绑定字段返回 `IDEMPOTENCY_CONFLICT`，同 `executionId` 改绑 merchant、settlement、channel 或 idempotency key 返回稳定 `SETTLEMENT_EXECUTION_IDENTITY_CONFLICT`，两类冲突均不消费 script、不创建第二 execution 或业务效果。
...
统一错误形态为 `ApiError { code, message, details, correlationId, retryable }`。至少支持 `VALIDATION_ERROR`、`NOT_FOUND`、`RESOURCE_NOT_READY`、`IDEMPOTENCY_CONFLICT`、`BUSINESS_CONFLICT`、`INVALID_STATE_TRANSITION`、`REFUND_BUDGET_EXCEEDED`、`UNRESOLVED_RECONCILIATION_DIFFERENCE`、`SETTLEMENT_SCOPE_CONFLICT`、`SETTLEMENT_EXECUTION_IDENTITY_CONFLICT`、`RESULT_UNKNOWN_REEXECUTION_FORBIDDEN`、`INVALID_CURSOR`、`REFERENCE_EXECUTOR_UNAVAILABLE`，并保留已有更细的稳定业务 code。HTTP status 可按 CAP4K 映射，但 `code/details/retryable` 与无副作用语义必须稳定；内部 SQL、类名、堆栈和 provider raw cause 不泄漏。未知权威账单使用稳定 `NOT_FOUND`（或更细但固定的 bill not-found code）并在 details 返回查询 billId，不得以空对象、Run 输入证据或 500 代替。
...
到期且无未决 attempt 的 PAYABLE 支付可关闭；存在 processing/unknown attempt 时不得判失败或关闭，而进入结果收敛/review。公开 `CloseExpiredPayment(merchantId,paymentId,idempotencyKey)` 只装载并裁决指定 Payment，复用同一聚合到期规则并返回真实 `OperationReceipt/READ_ONCE readAfter`；相同 payload 重放原 operation，merchant/payment 改绑稳定冲突。尚未到期或已成功支付同步 ApiError 且不创建 Operation。普通 scheduler 与 reference `PAYMENT_EXPIRY` maintenance 可复用领域规则，但不能替代该单资源命令或其 HTTP 验收。

支付成功时冻结当时实际生效的 `ReferencePolicy.feeRate`、固定费、`roundingMode`、币种精度及计算结果快照；默认 `feeRate=0.006`、`roundingMode=HALF_UP`，场景显式覆盖时必须使用覆盖值。后续 policy 或配置变化不追溯修改。首次成功形成稳定商户通知意图；重复/冲突结果不形成第二意图。
...
reference bill authority 保存 `AuthoritativeBill(billId, billIdentity, channelId, businessDate, businessTimezone, currency, currentRevision, createdAt)` 与不可变 `BillRevision`。`(channelId,billIdentity,revision)` 唯一；高 revision 推进指针，重复/低/迟到 revision 不回退。相同 revision 只有 fingerprint 与规范化完整内容一致时才幂等，任一绑定或内容不同均返回稳定冲突且不改写旧版。每个 revision 保存 `revision、publishedAt、completeness、rawEvidence、payloadFingerprint` 以及完整不可变 records；每条 record 保存稳定 `recordIdentity、transactionKind、externalTransactionIdentity、Money、rawStatus、occurredAt、receivedAt/recordedAt、rawEvidence`。暂不可读按同一 signal identity、policy 次数与逻辑 backoff 重试并保留诊断。

公开只读 `GetAuthoritativeBill` 与 `ListBillRevisions` 分别通过 `GET /api/authoritative-bills/{billId}` 和 `GET /api/authoritative-bills/{billId}/revisions` 暴露同一权威资源。两者均返回 current pointer 与严格按 `revision ASC` 排列的完整 history，不要求先运行对账，也不得从 `ReconciliationRun` 输入、前端缓存或 Controller 拼装。公开 billId 是主查询 identity，同时返回既有 billIdentity；为兼容 reference fixture，adapter 可把唯一 billIdentity 解析到同一资源，但不得形成第二套 bill。未知 bill 返回稳定 ApiError。
...
`ExecuteSettlement` 明确接收调用方提供的 `merchantId、settlementId、executionId、executionChannelId、idempotencyKey`。`executionId` 是持久化且全局稳定的公开执行 identity；`executionChannelId` 只是独立渠道 evidence，内部 execution attempt 主键、execution group 或 request identity 均不能替代它。完全相同绑定重放返回同一 execution/operation；任何 key 或 identity 改绑按 §2.4 稳定冲突。

reference settlement executor 必须在 Execute 提交边界按 `executionId` 原子消费一次已配置 script，并缓存同 identity 的原观察。`SUCCESS` 只形成一次 settled fact；`FAILURE` 保存诊断，仅明确失败后按显式命令/policy 使用新的 caller executionId；`UNKNOWN` 保存可信未知结果 evidence；`NO_RESULT` 保存已提交但未收到结果的诊断而不合成 callback。UNKNOWN/NO_RESULT 都保留原 identity，禁止新 identity、新 Settlement、void 或 replacement 重付，达到阈值后以相同 executionId 进入 `REVIEW_REQUIRED`。成功后迟到失败不回退；矛盾结果追加 receipt 与 review。

结算详情与 execution 列表、script observation、`ReceiveSettlementExecutionResult` callback、ManualReview 的 related/source refs 以及 payment timeline 必须引用完全相同的 caller-supplied `executionId`。callback 可以继续携带 result identity、occurredAt 与渠道 evidence，但不得接收另一个 attemptId 作为业务执行身份。
...
UNKNOWN、NO_RESULT、冲突、迟到、未知引用、未决差异、负净额等形成 `ManualReviewItem`，保存 type/status、related refs、blocking scopes、evidence、created/resolved 时间和全部追加处置。结算执行相关 review 的 related/source refs 必须使用 caller-supplied `executionId`，不得只暴露内部 attempt 主键。`ResolveManualReview` 由 §2.1 的可信 ReferenceActorContext 提供 actorId/role，由请求提供非空 reason/evidence/outcome；最终处置和查询暴露相同 actorId。context 缺失/未知或责任字段不完整时同步 `VALIDATION_ERROR` 且无副作用。提供权威列表、详情和处置历史。
...
按 paymentId 的权威 timeline 使用 `recordedAt ASC,eventId ASC` 稳定排序，同时保留外部 occurredAt，覆盖：支付创建与幂等重放、attempt、submission/result receipt、唯一成功/冲突/迟到、review/处置、退款/attempt/receipt/预算变化、通知/attempt、bill/revision/run/difference/disposition/confirmation、settlement item/execution/void/replacement 和人工动作。每个 settlement execution、script 结果、callback 与 review timeline entry 都引用同一 caller-supplied `executionId`。trace 来自后端跨聚合权威查询模型，不由 Controller 或前端拼装。
...
- settlement executor 提供按 caller-supplied executionId 的 reference-only configure/read/reset 控制面，支持 `SUCCESS/FAILURE/UNKNOWN/NO_RESULT`；空 executionId 或未知 script 返回稳定 ApiError；重复 configure/reset 幂等；Execute 实际消费一次并冻结首次 observation，read 返回配置/消费/观察诊断；identity 已消费后的 reconfigure/reset 不改写首次 consumption、observation、execution 或 evidence，旧 identity replay 仍返回原观察；reset 只能清除尚未消费的选择且不删除已形成 execution/evidence；
- notification sender 在稳定 identity/content 下脚本化 delivery success/failure；
- 相同外部 identity 不产生第二业务效果。

### 9.3 Stable fixture 与干净运行

reference profile 提供稳定 merchant/channel/configuration、场景 alias、policy、actor context、logical clock 和 executor scripts。actor fixture 只向调用方暴露 opaque session alias；其 actorId/role 映射保存在服务端 ReferenceActorRegistry，HTTP runner 通过 `X-Reference-Actor-Context` 选择 alias 并可从最终责任事实复核解析结果。测试可通过 fixture API/runner 控制逻辑时间和 sandbox 输入，但不能直接改领域表、直接决定 verification 或伪造服务端责任事实。settlement executor script 控制面只能选择确定性行为，不能自报 callback verification；Configure/Read/Reset 均由公开 reference-fixture HTTP surface 执行，script 消费和业务效果必须从公开 Settlement、Operation、review 与 timeline 查询复核。自动化和真实 HTTP 验收从干净 H2 运行启动；同 fixture 可使用不同随机内部 ID，但规范化 alias、状态、错误、关联、排序和副作用必须一致。
...
本 change brief 的 A1-A14 还必须在同一类干净 H2、启动真实服务、进程外 client 条件下逐项通过：Bill 查询不得由 Run 输入或缓存代替；CloseExpiredPayment 不得由全局 maintenance 代替；script 消费不得由显式 callback 或合成 receipt 代替；executionId 必须从 Execute 输入贯穿公开详情/callback/review/timeline。Controller 单测、后端内部对象或其他仓库改动均不算通过。
...
规范化命令覆盖：CreatePaymentIntent、CreatePaymentAttempt、SubmitPaymentAttempt、ReceivePaymentChannelResult、`CloseExpiredPayment(merchantId,paymentId,idempotencyKey)`、RequestRefund、Create/SubmitRefundAttempt、ReceiveRefundChannelResult、RefreshAuthoritativeBill、Run/RerunReconciliation、DisposeDifference、ConfirmFact、Prepare/Confirm/`ExecuteSettlement(merchantId,settlementId,executionId,executionChannelId,idempotencyKey)`/ReceiveResult/Void/ReplaceSettlement、ResolveManualReview、RetryMerchantNotification。reference-only 控制面另提供 settlement executor script Configure/Read/Reset，不把它伪装为资金业务命令。

规范化查询覆盖：GetOperation；Payment 详情/最终状态/attempts/receipts/list；Refund 详情/attempts/budget/list；`GetAuthoritativeBill(billId)` 与 `ListBillRevisions(billId)` 的 current pointer/完整 immutable history；Run 详情/list/differences/history；Settlement 详情/list/items/executions；ManualReview 详情/list；notifications；payment timeline。CAP4K 路由可重整，不保留旧 API compatibility facade。

### 10.5 验证分层

必须提供 domain invariant tests、H2/JPA repository/UoW/rollback/concurrency tests、Endpoint contract tests、HTTP binding/integration tests、query/pagination/cursor tests、trusted verifier/fixture tests、scheduler/event tests、完整 composition tests、覆盖全部 67 个 PAY-AC 的进程外真实 HTTP runner，以及 clean build、boot startup、Pipeline generation、Analyzer、AgentFacts 与 traceability guards。新增 A1-A14 必须另有干净 H2 进程外真实 HTTP 定向 runner，逐项证明 Bill history/raw evidence、单 Payment 到期 receipt、script configure/read/reset/消费、executionId 重放/冲突与全链关联。每个 PAY-AC 与 A1-A14 同时具有自动化证据和真实服务 HTTP 证据；最终证据不得以 focused test、MockMvc、合成 receipt、Run 输入证据或进程内规范化测试替代全量回归或真实 HTTP。
...
给定支付到期、PAYABLE 且无未决 attempt，当以绑定 merchantId/paymentId/idempotencyKey 的公开 CloseExpiredPayment 调用或普通到期扫描裁决时，则支付 CLOSED 且不再主动尝试；单资源命令返回真实 READ_ONCE OperationReceipt，相同请求重放原 operation。

#### Scenario: PAY-AC-008 到期但结果未知
给定到期支付存在 processing/unknown attempt，当调用单资源 CloseExpiredPayment 或扫描时，则不判失败/关闭并产生引用原 Payment/attempt 的核对事项；单资源命令不以全局 maintenance 回执代替。
...
给定 confirmed CNY 12700 settlement、caller-supplied executionId 和为该 identity 配置的 SUCCESS executor script，当 Execute 实际消费并形成匹配 SUCCESS 时，则详情、result evidence 与 timeline 使用相同 executionId，Settlement 为 SETTLED/FINAL，命令/script/结果重放均不形成第二事实。

#### Scenario: PAY-AC-064 结算执行结果未知
给定 caller-supplied executionId 已消费 UNKNOWN 或 NO_RESULT script，当 review 期限到达时，则保持原 executionId 并由详情/ManualReview/timeline 共同引用、进入 REVIEW_REQUIRED，禁止新 identity/新单/作废/替代/重付。
...
给定支付经历两 attempt、receipts、成功、部分退款/预算、通知、bill/revision、run/difference/disposition、settlement execution/callback/review，当按 paymentId 查询时，则完整 timeline 按 recordedAt/eventId 稳定排序，occurredAt/Money/identity 可核算，且结算条目使用 caller-supplied executionId 而非内部 attempt ID。
...
给定 bill/revision 可用且 scheduler、signal 重投、人工重跑并发乱序，当 provider 暂不可读后恢复时，则同 revision 只有一个有效 run，高 revision 前进、低 revision 不回退、同 signal identity 可诊断重试；公开 GetAuthoritativeBill/ListBillRevisions 始终返回 current pointer、完整 immutable history、fingerprint 与逐记录 raw evidence。
...
给定 bill revision 1/2、重复并发与迟到信号，当从新会话仅凭 billId 读取完整 revision 链并运行/重跑/查询时，则 Bill 可独立权威查询但只作输入证据，Run 是唯一对账执行主资源，高 revision 不回退、旧 run 保留且只有一个 effectiveRun，无并列 Batch API。
...
给定 confirmed settlement、caller-supplied executionId 和可经公开控制面 configure/read/reset 的 executor，当 Execute 分别消费 duplicate SUCCESS、明确 FAILURE 后显式使用新 executionId 重试、UNKNOWN 或 NO_RESULT 时，则成功一次、失败诊断/新 execution 受控、unknown/no-result 保持原 executionId 并禁重付；相同 identity 重放不再次消费 script。
...
给定固定 channel/bill/executor/notification scripts、fixture、policy 与 clock，当两个干净 H2 真实 HTTP 运行各完成同一闭环时，则 Bill detail/history、单 Payment 到期 receipt、caller executionId、状态、错误、关联、排序和副作用一致，同 identity replay 无第二效果。
...
- brief 的 A1-A14 必须逐项获得自动化 pass 与干净 H2、启动真实服务、进程外 client 的公开 HTTP pass 证据；Bill 查询、单 Payment 到期、script 消费和 executionId 全链不能由前端缓存、全局 maintenance、合成 receipt、Run 输入证据、显式 callback 替代 script 消费或后端内部对象代替。
