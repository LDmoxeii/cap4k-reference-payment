...
class PaymentHttpErrorAdviceTest {
    ...
    fun `uncontrolled argument and state messages never expose raw English diagnostics`() {
        ...
        assertEquals("VALIDATION_ERROR", badRequest.code)
    ...
    fun `persistence and unknown failures return fixed Chinese messages without internal details`() {
        ...
        val unknownResponse = advice.internalError(RuntimeException("provider stack and secret"))
        val unknown = requireNotNull(unknownResponse.body)
        ...
        assertEquals(500, unknownResponse.statusCode.value())
