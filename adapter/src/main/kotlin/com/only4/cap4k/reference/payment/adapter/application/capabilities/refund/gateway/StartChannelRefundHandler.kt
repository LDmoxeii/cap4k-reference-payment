...
class StartChannelRefundHandler : CapabilityHandler<StartChannelRefund.Request, StartChannelRefund.Response> {
    override fun call(request: StartChannelRefund.Request): StartChannelRefund.Response {
        ...
        if (request.channelId != "C-001") return StartChannelRefund.Response(
            false,
            null,
            "UNSUPPORTED_CHANNEL",
            "确定性 Fake 退款渠道仅接受 C-001"
        )
        return StartChannelRefund.Response(
            true,
            "fake-refund-${request.requestIdentity}",
            null,
            "确定性 Fake 退款渠道已受理请求"
        )
    }
